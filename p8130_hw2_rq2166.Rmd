...
```{r }
d_i   = mean(BMI_i$BMI_POST - BMI_i$BMI_PRE)
sd_i  = sd((BMI_i$BMI_POST - BMI_i$BMI_PRE))
...
p_value = 2*pt(t_s,n-1)
p_value

# calculate the confidence interval
CI = paste(round(d_i-qt(0.975,n-1)*sqrt(sd_i^2/n),5),
           round(d_i+qt(0.975,n-1)*sqrt(sd_i^2/n),5))
CI
```
...
Paired-t.test calculating step by step using formulae
```{r }
d_c   = mean(BMI_c$BMI_POST-BMI_c$BMI_PRE)
sd_c  = sd(BMI_c$BMI_POST-BMI_c$BMI_PRE)
...
p_value = 2*(1-pt(t_s,n-1))
p_value

# calculate the confidence interval
CI = paste(round(d_c-qt(0.975,n-1)*sqrt(sd_c^2/n),5),
           round(d_c+qt(0.975,n-1)*sqrt(sd_c^2/n),5))
CI
```
## b)
...
p_value = 2*pt(t_s,round(df))
p_value

# calculate the confidence interval
CI = paste(round(d_i-d_c-qt(0.975,round(df))*
                   sqrt(sd_i^2/n+sd_c^2/n),5),
           round(d_i-d_c+qt(0.975,round(df))*
                   sqrt(sd_i^2/n+sd_c^2/n),5))
CI
```
...
d  = with(sleep,sum((extra[group == 2]-extra[group == 1]))/(n))
sd = with(sleep,sd  (extra[group == 2]-extra[group == 1]))
...
```{r}
# calculate the confidence interval
CI = paste(round(d-qt(0.95,n-1)*sqrt(sd^2/n),5),"Inf")
CI
```
...
power=
  pt(-qt(1-0.05,n-1) + delta/(sd/sqrt(n)),n-1)
